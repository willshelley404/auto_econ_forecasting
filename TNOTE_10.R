...
# #  value  ----
value_reg <- fredr(series_id = "GS10") %>% # 
...
value_reg <- value_reg %>%
  ...
  tk_augment_leads(value, .lags = 1:4) %>%
  na.omit()
...
xreg_vars <-  forecast_data %>% 
  select(date, value, value_lag1, value_lag2, value_lag3, year, value_inv1)
...
lm_fit <- lm_mod %>% fit(value ~ value_lag1 + value_lag2 + value_lag3 + value_inv1, data = xreg_vars)
...
new_points <- xreg_vars %>% 
  ...
  select(year, value_lag1, value_lag2, value_lag3, value_lag4, value_inv1)
...
for (i in 1:60) {
  ...
  data <- output %>% 
    ...
    na.omit() %>% 
    mutate(year = year(date),
           value_inv1 = -(1/value_lag1))   # REMOVE NEGATIVE IF NEEDED
  
  # fit new mod
  lm_fit <- lm_mod %>% fit(value ~ value_lag1 + value_lag2 + value_lag3 + value_inv1, data = data)

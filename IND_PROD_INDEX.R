...
# #  value  ----
value_reg <- fredr(series_id = "INDPRO") %>% # 
...
xreg_vars <-  forecast_data %>% 
  select(date, value, value_lag1, value_lag3, value_inv1, year)
...
lm_fit <- lm_mod %>% fit(value ~ value_lag1 + value_lag3 + year + value_inv1, data = xreg_vars)
...
for (i in 1:60) {
  ...
  # fit new mod
  lm_fit <- lm_mod %>% fit(value ~ value_lag1 + value_lag3 + value_inv1 + year, data = data)
...
ind_prod_index <- output %>% 
  rename(ind_prod_index = value)

...
# #  value  ----
value_reg <- fredr(series_id = "A229RX0") %>% # 
...
xreg_vars <-  forecast_data %>% 
  select(date, value, value_lag3, year)
...
lm_fit <- lm_mod %>% fit(value ~ value_lag3 + year, data = xreg_vars)
...
new_points <- xreg_vars %>% 
  ...
  select(year, value_lag1, value_lag2, value_lag3)
...
for (i in 1:60) {
  ...
  # fit new mod
  lm_fit <- lm_mod %>% fit(value ~ value_lag3, data = data)
...
income_per_cap <- output %>% 
  rename(income_per_cap = value)

...
# #  value  ----
value_reg <- fredr(series_id = "PAYEMS") %>% # CE16OV   PAYEMS
...
xreg_vars <-  forecast_data %>% 
  select(date, value, value_inv1, value_lag1,value_lag2,value_lag3,value_lag4,year)
...
lm_fit <- lm_mod %>% fit(value ~ value_lag1 +  value_lag4 + year + value_inv1, data = xreg_vars)
...
for (i in 1:60) {
  ...
  # fit new mod
  lm_fit <- lm_mod %>% fit(value ~ value_lag1 + value_lag2 + value_lag3 + value_lag4 + year + value_inv1, data = data)

...
xreg_vars <-  forecast_data %>% 
  select(date, value, value_lag2, value_lag4, year)
...
lm_fit <- lm_mod %>% fit(value ~  value_lag2 + value_lag4 + year, data = xreg_vars)
...
new_points <- xreg_vars %>% 
  ...
  select(year, value_lag1, value_lag2, value_lag3, value_lag4)
...
for (i in 1:60) {
  ...
  # fit new mod
  lm_fit <- lm_mod %>% fit(value ~ value_lag2 + value_lag4 + year, data = data)
